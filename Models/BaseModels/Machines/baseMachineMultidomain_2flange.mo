...
partial model baseMachineMultidomain_2flange
  ...
  Modelica.Units.SI.Torque T_sum "sum of torques at both flanges [pu]";
protected
  parameter
...
equation
  T_sum = flange_a.tau + flange_b.tau;
  ...
  flange_a.phi = delta*pole_no/2; // (deg)
  (SPEED_MECH*T_sum/(M_b))*(pole_no/2) = PMECH; // (pu)
  ...
  annotation (
    Icon(coordinateSystem(preserveAspectRatio=false, extent={{-100,-100},{100,
          ...
          lineThickness=0.5,
          textString="EFD")}));
end baseMachineMultidomain_2flange;
